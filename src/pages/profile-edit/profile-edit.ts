...
export class ProfileEdit extends Block<ProfileEditProps, ProfileEditState> {
	...
	protected initChildren() {
				...
				this.childrens[`userFields ${key}`] = new ProfileEditCell({
					...
					input: new InputField({
						...
						onChange: (e: Event) => {
							...
							this.childrens[`userFields ${key}`].setProps({
								error: fieldErrors,
								value: input.value,
							});
		...
		this.childrens.avatar = new Avatar({
			user_src:
				CONSTATNS.BASE_SOURCES_URL + window.store.getState().user?.avatar ||
				'img/avatar_mock.jpg',
			className: 'avatar cursor-pointer mb-2',
			width: 130,
			onClick: () => {
				window.store.set({ openedModal: 'uploadAvatar' });
			},
		});
	...
	private handleSave() {
		...
		(
			Object.keys(this.state) as Array<keyof Omit<ProfileEditState, 'errors'>>
		).forEach((key) => {
			const fieldErrors =
				Validator.validate({ [key]: this.state[key] }, this.fieldLabels)[key] ||
				[];
